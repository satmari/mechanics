        ...
        <div class="text-center">
            <div class="panel panel-default">
                ...
                <div class="panel-heading">
                    <div class="row">
                        <div class="col-md-6"><h3>Guides Table</h3></div>
                        <div class="col-md-6 text-right">
                            <a href="{{ url('guides_add') }}" class="btn btn-success btn-add">Add new Guide</a>&nbsp;&nbsp;
                            <a href="{{ url('guides_transfer') }}" class="btn btn-warning btn-add">Transfer</a>&nbsp;&nbsp;
                            <a href="{{ url('guides_add_remove') }}" class="btn btn-danger btn-add">Add or Reduce from stock</a>&nbsp;&nbsp;
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-striped table-bordered tableFixHead" id="sort"
                        ...
                        <thead>
                            ...
                            <tr>
                                ...
                                <th><input type="text" class="form-control form-control-sm column-filter" data-column="7" placeholder="Filter…"></th>
                                <th><input type="text" class="form-control form-control-sm column-filter" data-column="8" placeholder="Filter…"></th>
                                <th><input type="text" class="form-control form-control-sm column-filter" data-column="9" placeholder="Filter…"></th>
                                <th><input type="text" class="form-control form-control-sm column-filter" data-column="10" placeholder="Filter…"></th>
                                <th><input type="text" class="form-control form-control-sm column-filter" data-column="11" placeholder="Filter…"></th>
                        ...
                        <tbody class="searchable">
                            ...
                            <tr title="Operation: {{ $req->operation }}&#10;Note: {{ $req->notes }}">
                                <td class="red-text">{{ $req->guide_code }}</td>
                                <!-- <td>{{ $req->guide_description }}</td> -->
                                <td>{{ $req->machine_class }}</td>
                                <td>{{ $req->fold }}</td>
                                <td>{{ $req->entry_mm }}</td>
                                <td>{{ $req->exit_mm }}</td>
                                <td>{{ $req->tickness_mm }}</td>
                                <td>{{ $req->elastic_mm }}</td>
                                <td>{{ $req->style }}</td>
                                <td>{{ $req->operation }}</td>
                                <td>{{ $req->notes }}</td>
                                <td>{{ $req->location }}</td>
                                ...
                                <td>
                                    @if($req->picture)
                                        <a href="javascript:void(0);" 
                                           onclick="window.open(
                                               '{{ asset('public/storage/GuidesFiles/'.$req->picture) }}', 
                                               '_blank',
                                ...
                                <td>
                                    @if($req->video)
                                        <a href="javascript:void(0);" 
                                           onclick="window.open(
                                               '{{ asset('public/storage/GuidesFiles/'.$req->video) }}', 
                                               '_blank',
                                               'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=800,height=600'
                                           )">View</a>
                                    @endif
                                ...
                                <td>
                                    <a href="{{ url('guide_edit/'.$req->id) }}" class="btn btn-primary btn-xs">Edit</a>
                                </td>

                                <td>
                                    <a href="{{ url('guide_history/'.$req->id) }}" class="btn btn-info btn-xs">History</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
